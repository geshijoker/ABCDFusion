...
python examples/single_modality_cv.py -c ./configs.yaml -e ./exps/DTI -n 0 -m DTI -l bce -s 42 -g -1 -f 0 -ne 10 -lr 0.0001 -b -d
"""
...
parser.add_argument('--name', '-n', type=str, required=True, 
                    help='name of run', )
parser.add_argument('--modality', '-m', type=str, required=True,
...
for i, (train_index, test_index) in enumerate(splitter.split(*X, y, groups)):
    print(f"Fold {i}; Sites {set(groups[test_index])}; There are {len(train_index)} training subjects and {len(test_index)} testing subjects.")
    input_size = len(X[0][0])
    
    if model_params['architecture'] == 'mlp':
        model = nn.Sequential(
            BinaryMLP(input_size, model_params['layers']),
            LinearClassifier(model_params['layers'][-1], p=model_params['dropout'])
        )
    ...
    optimizer = getattr(optim, model_params['optimizer'])(model.parameters(), lr=model_params['lr'])
    start_epoch = 0
